...
namespace PS4wpfDumper.PFS
{
    ...
    public class PfsBuilder
    {
        ...
        public PfsBuilder(PfsProperties p, System.Action<string> logger = null)
        {
            this.logger = logger;
            properties = p;
            Setup();
        ...
        public void WriteImage(MemoryMappedFile file, long offset)
        {
            ...
            using (var view = file.CreateViewAccessor(offset, CalculatePfsSize(), MemoryMappedFileAccess.ReadWrite))
            {
                if (hdr.Mode.HasFlag(PfsMode.Signed))
                {
                    ...
                    var signKey = Util.Crypto.PfsGenSignKey(properties.EKPFS, hdr.Seed);
                    // We can do the actual data blocks in parallel
                    Parallel.ForEach(
                      data_sigs,
                      () => System.Tuple.Create(new byte[properties.BlockSize], new HMACSHA256(signKey)),
                      (sig, status, local) =>
                      {
                          ...
                          view.WriteArray(position, Util.Crypto.HmacSha256(signKey, sig_buffer), 0, 32);
                ...
                if (hdr.Mode.HasFlag(PfsMode.Encrypted))
                {
                    ...
                    var (tweakKey, dataKey) = Util.Crypto.PfsGenEncKey(properties.EKPFS, hdr.Seed);
                    Parallel.ForEach(
                      // generates sector indices for each sector to be encrypted
                      XtsSectorGen(),
                      // generates thread-local data
                      () => System.Tuple.Create(new Util.XtsBlockTransform(dataKey, tweakKey), new byte[xtsSectorSize]),
                      // Loop body
        ...
        public void WriteImage(Stream stream)
        {
            ...
            if (hdr.Mode.HasFlag(PfsMode.Signed))
            {
                ...
                foreach (var sig in data_sigs.Concat(final_sigs))
                {
                    ...
                    stream.Write(Util.Crypto.HmacSha256(signKey, sig_buffer), 0, 32);
            ...
            if (hdr.Mode.HasFlag(PfsMode.Encrypted))
            {
                Log("Encrypting...");
                var (tweakKey, dataKey) = Util.Crypto.PfsGenEncKey(properties.EKPFS, hdr.Seed);
                var transformer = new Util.XtsBlockTransform(dataKey, tweakKey);
        ...
        void CalculateDataBlockLayout()
        {
            ...
            hdr.Ndblock = System.Math.Max(hdr.Ndblock, properties.MinBlocks);
        }
