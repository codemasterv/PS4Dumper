...
namespace PS4wpfDumper.PFS
{
    ...
    public class PfsReader
    {
        ...
        {
            public int[] blocks;
            private Util.IMemoryReader reader;
            public File(Util.IMemoryReader r) { reader = r; }
            public Util.IMemoryReader GetView()
            {
                if (blocks != null)
                    return new Util.ChunkedMemoryReader(reader, 0x10000, blocks);
                return new Util.MemoryAccessor(reader, offset);
            }
            public void Save(string path, bool decompress = false)
            {
                ...
                using (var file = System.IO.File.OpenWrite(path))
                {
                    ...
                    while (sz > 0)
                    {
                        var toRead = (int)System.Math.Min(sz, buf.Length);
        ...
        : this(new Util.MemoryMappedViewAccessor_(r), pfs_flags, ekpfs, tweak, data)
        { }
        public PfsReader(Util.IMemoryReader r, ulong pfs_flags = 0, byte[] ekpfs = null, byte[] tweak = null, byte[] data = null)
        {
            reader = r;
            ...
            int dinodeSize;
            System.Func<Stream, inode> dinodeReader;
            ...
            if (hdr.Mode.HasFlag(PfsMode.Encrypted))
            {
                ...
                if (ekpfs == null && (tweak == null || data == null))
                    throw new System.ArgumentException("PFS image is encrypted but no decryption key was provided");
                if (ekpfs != null)
                {
                    var (tweakKey, dataKey) = Util.Crypto.PfsGenEncKey(ekpfs, hdr.Seed, (pfs_flags & 0x2000000000000000UL) != 0);
            ...
            if (uroot == null)
                throw new System.Exception("Invalid PFS image (no uroot)");
            uroot.name = "uroot";
        }
        ...
        private File LoadFile(uint dinode, Dir parent, string name)
        {
            ...
            if (dinodes[dinode].Blocks > 1 && dinodes[dinode].DirectBlocks[1] != -1)
            {
                if (!hdr.Mode.HasFlag(PfsMode.Signed))
                {
                    throw new System.Exception("Unsigned PFS images probably shouldn't have noncontiguous blocks");
                }
                ...
                var bufferedReader = new Util.BufferedMemoryReader(reader, 0x10000);
                remainingBlocks -= 12;
