...
namespace PS4wpfDumper.Util//
{
    public static class Crypto
    {
        ...
        public static byte[] PfsGenCryptoKey(byte[] ekpfs, byte[] seed, uint index)
        {
            ...
            System.Array.Copy(System.BitConverter.GetBytes(index), d, 4);
            System.Array.Copy(seed, 0, d, 4, seed.Length);
        ...
        /// </summary>
        public static System.Tuple<byte[], byte[]> PfsGenEncKey(byte[] ekpfs, byte[] seed, bool newCrypt = false)
        {
            ...
            System.Buffer.BlockCopy(encKey, 0, tweakKey, 0, 16);
            System.Buffer.BlockCopy(encKey, 16, dataKey, 0, 16);
            return System.Tuple.Create(tweakKey, dataKey);
        }
        ...
        public static byte[] RSA2048EncryptKey(byte[] modulus, byte[] hash)
        {
            ...
            System.Buffer.BlockCopy(modulus, 0, buffer, 0, 256);
            System.Buffer.BlockCopy(hash, 0, buffer, 256, 32);
            ...
            padded_input[1] = 2;
            padded_input[223] = 0;
            System.Buffer.BlockCopy(hash, 0, padded_input, 224, 32);
            for (int k = 2; k < 223;)
            {
        ...
        public static int AesCbcCfb128Encrypt(byte[] @out, byte[] @in, int size, byte[] key, byte[] iv)
        {
            ...
            System.Buffer.BlockCopy(tmp, 0, @out, 0, tmp.Length);
            return 0;
        }
        ...
        public static byte[] ComputeKeys(string ContentId, string Passcode, uint Index)
        {
            if (ContentId.Length != 36)
                throw new System.Exception("Content ID must be 36 characters long");
            if (Passcode.Length != 32)
                throw new System.Exception("Passcode must be 32 characters long");

            byte[] data = new byte[96];
            System.Buffer.BlockCopy(Sha256(System.BitConverter.GetBytes(Index).Reverse().ToArray()), 0, data, 0, 32);
            System.Buffer.BlockCopy(Sha256(Encoding.ASCII.GetBytes(ContentId.PadRight(48, '\0'))), 0, data, 32, 32);
            System.Buffer.BlockCopy(Encoding.ASCII.GetBytes(Passcode), 0, data, 64, 32);
