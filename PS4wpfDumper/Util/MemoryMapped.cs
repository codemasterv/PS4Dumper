...
namespace PS4wpfDumper.Util
{
  static class MemoryReaderExtensions
  {
    ...
    private static unsafe T ByteArrayToStructure<T>(byte[] bytes, int offset) where T : struct
    {
      ...
      {
        return (T)Marshal.PtrToStructure((System.IntPtr)ptr, typeof(T));
      }
    }
  }
  ...
  public interface IMemoryReader : System.IDisposable
    {
  ...
  class ChunkedMemoryReader : IMemoryReader
  {
    ...
    public void Read(long pos, byte[] buf, int offset, int count)
    {
      ...
      while(count > 0)
      {
        ...
        int toReadFromChunk = System.Math.Min(chunkSize - offsetIntoChunk, count);
  ...
  class BufferedMemoryReader : IMemoryReader
  {
    ...
    public void Read(long pos, byte[] buf, int offset, int count)
    {
      while (count > 0 && pos > 0)
      {
        ...
        int toReadFromChunk = System.Math.Min(buffer.Length - offsetIntoBuffer, count);
                System.Buffer.BlockCopy(buffer, offsetIntoBuffer, buf, offset, toReadFromChunk);
        pos += toReadFromChunk;
        offset += toReadFromChunk;
        count -= toReadFromChunk;
      }
