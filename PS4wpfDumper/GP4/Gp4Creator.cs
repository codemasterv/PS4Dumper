...
namespace PS4wpfDumper.GP4
{
  /// <summary>
  /// Contains functionality to create a GP4 from a PKG
  /// </summary>
  public class Gp4Creator
  {
    ...
    public static EntryId[] GeneratedEntries = new[]
    {
      ...
      EntryId.PLAYGO_MANIFEST_XML,
        ...
        public static void CreateProjectFromDirectory(string outputDir, MemoryMappedFile pkgFile, string passcode = null)
        {
            Directory.CreateDirectory(outputDir);

            // Since we're dealing with directories now, we can't extract package metadata from PKG headers.
            // These parameters may need to be passed as arguments or handled differently.
            passcode = passcode ?? "00000000000000000000000000000000";

            // Initialize project parameters
            var project = Gp4Project.Create(VolumeType.pkg_ps4_app); // Adjust as necessary
            project.volume.Package.Passcode = passcode;
            project.volume.Package.ContentId = CONTENT_ID; // Adjust as necessary
            project.volume.Package.AppType = "full"; // Adjust as necessary
            project.volume.Package.StorageType = PUBTOOLINFO_st_type; // Adjust as necessary
         ...
         // Process the source directory
            ...
            var allFiles = Directory.GetFiles($@"ext\CUSA07023-app0\", "*.*", SearchOption.AllDirectories);   ////////////////////////////////

            foreach (string filePath in allFiles)
            {

                string sourceDir = $@"ext\CUSA07023-app0\";
                
                var entryName = filePath.Replace(sourceDir, "").TrimStart('\\');
                var filename = Path.Combine(outputDir, entryName);

                // Add the entry to the project
                project.files.Items.Add(new Gp4File()
                {
                    OrigPath = entryName.Replace("\\", "/"),
                    TargetPath = entryName.Replace("\\", "/")
                });
            ...
            // Since we're not dealing with a PKG, we don't extract files from a PFS filesystem.
            // Hence, the logic for extracting files from the PFS filesystem has been removed.

            // Last step: save the project file
            using (var f = File.Create(Path.Combine(outputDir, "Project2.gp4")))
            {
                Gp4Project.WriteTo(project, f);
            }
        }

        public static void fixSFO(MainWindow main)
        {
            // Fixup the param.sfo (Assuming that your source directory contains the necessary files)
            /*using (var f = File.Open(Path.Combine($@"ext/" + TITLE_ID + type, "sce_sys/param.sfo"), FileMode.Open))
            {
                //var Value = "0x204";
        ...
        public static async Task CreateProjectFromDIRAsync(string outputDir, string pkgFilename, string passcode)
        {
            ...
            string[] fileEntries = Directory.GetFiles(pkgFilename);
            var tasks = fileEntries.Select(fileName =>
                Task.Run(() =>
                {
                    using (var pkgFile = MemoryMappedFile.CreateFromFile(fileName, FileMode.Open))
                    {
                        CreateProjectFromDirectory(outputDir, pkgFile, passcode);
                    }
                }));
        ...
        public static void CreateProjectFromPKG(string outputDir, MemoryMappedFile pkgFile, string passcode = null)
    {
      ...
      {
        ekpfs = Util.Crypto.ComputeKeys(pkg.Header.content_id, passcode, 1);
      }
      else
      {
        ekpfs = pkg.GetEkpfs();
    ...
    public static void CreateProjectFromPKG1(string outputDir, string pkgFilename, string passcode = null)
    {
      using (var pkgFile = MemoryMappedFile.CreateFromFile(pkgFilename, FileMode.Open))
      {
        CreateProjectFromPKG(outputDir, pkgFile, passcode);
